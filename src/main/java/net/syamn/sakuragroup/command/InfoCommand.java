/**
 * SakuraGroup - Package: net.syamn.sakuragroup.command Created: 2012/10/26
 * 6:46:37
 */
...
public class InfoCommand extends BaseCommand {
    ...
    private List<String> buildStrings(final String name, final boolean other) {
        ...
        HashMap<Integer, ArrayList<String>> result = db.read("SELECT `player_id`, `group`, `status`, `changed`, `lastchange`, `lastpaid` FROM " + db.getTablePrefix() + "users WHERE `player_name` = ?", name);
        ...
        if (other) l.add("&aプレイヤー: &6" + name + " &7(ID:" + player_id + ")");
