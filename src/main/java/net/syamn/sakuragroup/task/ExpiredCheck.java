...
public class ExpiredCheck implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            HashMap<Integer, ArrayList<String>> result = db.read("SELECT `player_name`, `group`, `status` FROM " + db.getTablePrefix() + "users WHERE `lastpaid` <= ? AND `group` != ?", threshold.intValue(), defGroup);
